...
class LocalArray {
public:
 LocalArray(const size_t sizeParam, Arena& arena = Arena::getArena()):
  _size(sizeParam), // parameter called sizeParam to avoid aliasing size()
  _range(arena.allocArray<T>(size)) {
 ...
 private:
 static const std::pair<T*, T*> gccWorkAround(const size_t size) {
